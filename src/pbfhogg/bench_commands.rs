...
mod tests {
    ...
    #[test]
    fn exact_match_diff_osc() {
        let result = parse_command("diff-osc").unwrap();
        assert_eq!(result, vec!["diff-osc"]);
    }
    ...
    fn prefix_cat_expands() {
        ...
        assert_eq!(result, vec!["cat-way", "cat-relation", "cat-dedupe"]);
    }

    #[test]
    fn prefix_inspect_expands() {
        // "inspect" is an exact match — exact match wins over prefix expansion.
        let result = parse_command("inspect").unwrap();
        assert_eq!(result, vec!["inspect"]);
    }
    ...
    fn prefix_check_expands() {
        // "check" is not an exact match, so prefix expansion kicks in.
        let result = parse_command("check").unwrap();
        assert_eq!(result, vec!["check-refs", "check-ids"]);
    }
    ...
    #[test]
    fn prefix_d_matches_diff_commands() {
        let result = parse_command("d").unwrap();
        assert!(result.contains(&"diff"));
        assert!(result.contains(&"diff-osc"));
    }
    ...
    fn exact_match_takes_priority_over_prefix() {
        // "getid" is both an exact match and a prefix of "getid-invert".
