...
const NEEDS_OSC: &[&str] = &["tags-filter-osc", "merge-changes"];

/// Commands that produce `.osc.gz` output instead of `.osm.pbf`.
const OSC_OUTPUT: &[&str] = &["tags-filter-osc", "merge-changes"];
...
fn command_args(name: &str, pbf: &str, merged_pbf: Option<&str>, osc: Option<&str>, output: &str) -> Result<Vec<String>, DevError> {
    let args = match name {
        ...
        "tags-filter-osc" => {
            ...
            vec!["tags-filter-osc".into(), o.into(), "highway=primary".into(), "-o".into(), output.into()]
        }
        ...
        "removeid" => vec!["removeid".into(), pbf.into(), "n115722".into(), "n115723".into(), "n115724".into(), "w2080".into(), "w2081".into(), "w2082".into(), "r174".into(), "r213".into(), "r339".into(), "-o".into(), output.into()],
        "renumber" => vec!["renumber".into(), pbf.into(), "-o".into(), output.into()],
        "merge-pbf" => vec!["merge-pbf".into(), pbf.into(), pbf.into(), "-o".into(), output.into()],
        "merge-changes" => {
        ...
        "derive-changes" => {
            ...
            vec!["derive-changes".into(), pbf.into(), m.into(), "-o".into(), output.into()]
        }
...
pub fn run(
    ...
    let osc_str = if needs_osc(commands) {
        let osc = osc_path.ok_or_else(|| {
            DevError::Config("tags-filter-osc/merge-changes require an OSC file (dataset must have osc configured)".into())
        })?;
...
mod tests {
    ...
    fn prefix_tags_filter_expands() {
        ...
        assert_eq!(
            result,
            vec!["tags-filter-way", "tags-filter-amenity", "tags-filter-twopass", "tags-filter-osc"]
        );
    }
