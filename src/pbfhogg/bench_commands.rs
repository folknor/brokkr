...
/// Commands that produce real output files and need a scratch path (not /dev/null).
const NEEDS_SCRATCH_OUTPUT: &[&str] = &["add-locations-to-ways"];
...
fn command_args(name: &str, pbf: &str, merged_pbf: Option<&str>, scratch_output: Option<&str>) -> Result<Vec<String>, DevError> {
    let args = match name {
        ...
        "add-locations-to-ways" => {
            let output = scratch_output.unwrap_or("/dev/null");
            vec!["add-locations-to-ways".into(), pbf.into(), "-o".into(), output.into()]
        }
...
pub fn run(
    ...
    // Ensure scratch dir exists if any command needs scratch output.
    if commands.iter().any(|c| NEEDS_SCRATCH_OUTPUT.contains(c))
    ...
    for &name in commands {
        ...
        // Compute scratch output path for commands that produce real output files.
        let scratch_output_path = if NEEDS_SCRATCH_OUTPUT.contains(&name) {
            scratch_dir.map(|sd| sd.join(format!("bench-{name}-output.osm.pbf")))
        } else {
            None
        };
        let scratch_output_str = scratch_output_path.as_ref().and_then(|p| p.to_str());

        let args = command_args(name, pbf_str, merged_str, scratch_output_str)?;
