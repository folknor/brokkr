...
- `docs/commands/check.md` - read if working on `brokkr check` or `brokkr test`, the gremlins/clippy/test pipeline, sweep selection, profile resolution, libtest filters, the `parallel = { budget = N }` concurrent-test-binaries lane (`src/check_cmd/parallel.rs`: the in-flight budget, why it counts tests rather than binaries, why the serial side is a second `[[check]]` entry rather than a key, and the direct execution of prebuilt binaries under cargo's reconstructed launch contract - `src/check_cmd/direct_runtime.rs` - which makes the fan-out legal on any selection shape, `test_exclude_packages` included; the whole-workspace `-Zfeature-unification` promotion still shapes the prebuild), the `[[check]] doc_only` doctest twin (a `cargo test --doc`-only sweep, the doctest carrier a parallel workspace sweep needs, with the workspace-shaped-carrier rule complete profiles enforce), the `[[check]] feature_unification` key (`auto`/`selected`/`workspace`/`package`, resolved once per invocation by `profile::resolve_unification` and stored on the sweep as `effective_unification`, which puts it in `build_shape_key`; `package` is the `cargo install` boundary, resolves one cargo invocation per package across every phase, isolates its target dir, audits one coverage shape per resolution, and refuses forwarded package selectors), the `[[check]] harness = "nextest"` lane (`src/check_cmd/nextest_lane.rs`: the linked engine as a fast process-per-test EXECUTOR under a brokkr-synthesized config - the checkout's `.config/nextest.toml` is never opened, brokkr owns compile shape, filters, concurrency, and the 20s watchdog rides the synthesized slow-timeout; the coverage audit covers nextest lanes - `src/check_cmd/nextest.rs` holds the `(binary-id, test)` pair and disposition classifier, the finer key applies shape-wide on mixed shapes, and complete profiles may reference nextest sweeps), the install-feature phase (`src/check_cmd/install_shape.rs`: the `[bin] install` set compiled under package-mode feature unification, the way `cargo install` resolves it), the time ceilings (`src/check_cmd/watchdog.rs`: 25 min on the whole run plus per-phase limits - clippy 5, test 15 - armed after the lock is taken, re-armed by `begin_phase`, firing as `brokkr kill --hard` would - SIGKILL every `/proc` descendant, exit 124; plus the libtest runner's per-test HARD CAP - every test brokkr runs gets 20s of wall time and no more, and exceeding it fails the run and STOPS it, in `check` and `brokkr test` alike; the sole exception is `brokkr test --timeout`, hard-limited to 280s. `Ceilings` in `src/test_runner.rs` carries the cap plus a wall backstop and a `WallShape` that decides only what a kill may be *named* - exact for a caller-resolved single test, the tracker's best-known suspect on a shared harness, since a libtest record swallowed by test output can blur the name but never excuse the overrun. Plus the 5-minute idle ceiling, `IDLE_TIMEOUT`, for the cargo-wedged-before-the-first-test case no test can be billed for), the rustc guard (`src/bin/rustc_guard.rs` + `src/guard.rs` + `src/hold.rs`: the `build.rustc-wrapper` fence `brokkr guard --install` enrolls user-wide, admitting a compile only when it carries the current hold's capability or `BROKKR_CARGO=1`; admitted compilation holds a reader lease on `compile.lock` that a new acquisition drains before measuring, so acquisition can now fail with a drain error, and the exclusion guarantee is conditional on participation), or the `BROKKR_TEST_BIN_DIR` contract.
...
- `src/check_cmd/binary_timings.rs` - `.brokkr/parallel-timings.toml`: per-binary serial cost (sum of its own tests) and slowest single test, carried between runs so a `parallel` sweep allocates threads by measured cost rather than test count. Serial cost specifically, not wall time: wall time is a function of the slots granted, so feeding it back oscillates (measured as a two-cycle). `slowest` gives the cap where extra threads stop helping, since no binary beats its longest test. Advisory in every direction - a missing or stale file costs one warm-up run, never a wrong result
- `src/cpu_topology.rs` - physical cores sharing one last-level cache, read from sysfs (`cache/index3/shared_cpu_list` collapsed onto `topology/core_id`). The default in-flight budget for a `parallel` `[[check]]` sweep, and `brokkr env`'s `l3 domain:` line. Not logical CPUs: SMT siblings contend for one core and cross-chiplet pairs pay fabric latency, so `nproc` would overstate the machine. On AMD the domain is a CCX/CCD, on Intel the die - one metric, two names, `domain_label` picks the word
- `src/test_runner.rs` - the libtest runner behind `check`'s test phase and `brokkr test`. **Every lane drives libtest's JSON event stream** (`-Z unstable-options --format json`, so nightly is a prerequisite); `JsonReconstructor` renders the events back to human text for the downstream parsers and `--raw`. The partial-marker state machine that used to reconstruct which test was running from a `test NAME ... ` marker glued to the test's own output is deleted - see the note where it stood for the two bugs it never fully fixed. `TestTracker` charges the per-test budget from those events, with a second no-progress clock for a lost start record and refreshes counted once per name per suite so a forged record buys nothing
- `src/test_scratch.rs` - the one scratch-dir allocator for unit tests (`scratch` for a directory, `scratch_path` for a file path). Asserts each `(module, name)` is used once: the allocator deletes before returning, so a shared name means two tests delete each other's files - invisible serially, an intermittent failure under concurrency. Exists because that precondition failed twice, both times a *shared helper* naming its own dir rather than a test picking a duplicate
- `src/globs.rs` - `globset` wrapper for the path-glob lists shared by `header`/`textlint`
- `src/scope.rs` - Scope + limit helpers. `changed_files()` computes files modified on the current branch via git merge-base; `dirt()` classifies the working tree (`Unknown`/`Clean`/`ProseOnly`/`Code`) for `check`'s markdown-only shortcut; `partition()` sorts diagnostics scoped-first; `format_trailer()` builds the overflow summary
...
- `src/lockfile.rs` - `LockGuard` (via `OwnedFd`); the lock file is always `$HOME/.brokkr/brokkr.lock`, one path with no alternatives and no env override - it used to prefer `$XDG_RUNTIME_DIR`, which logind sets per session and sandboxes repoint freely, so two invocations could hold two different files and neither excluded the other; not `~/.cache/` either, since a swept cache dir unlinks the file out from under a live hold. `acquire` is re-entrant within the process (nested acquires share one flock hold, released when the last guard drops) - what lets ratatoskr's `--gate all` cohort hold the lock across a whole sweep without self-deadlocking on a second flock fd (`sync --all` holds sweep-wide too but no longer nests - it builds once and runs scripts against the prebuilt harness). The flock is the sole authority on held/not-held; every PID in the lock file is namespace-local (a sandboxed holder writes pid=2), so each carries a starttime identity token plus a shared boot id, and readers (`brokkr lock`, the waiting message, `kill`) verify the token against this namespace's `/proc` before displaying stats or signalling - failing closed ("identity could not be verified") on any mismatch. Metadata is truncated to zero under the flock on release and on failed updates so handoff readers fail closed too; the format always emits every key, parses first-occurrence-wins, and escapes textual values
- `src/stray.rs` - the stray-cargo reaper: `/proc` scan for cargo-family processes with no brokkr ancestor, starter attribution, leaves-first SIGKILL (rust-analyzer starters included). Run by every locked command after acquisition and by `brokkr strays`
- `src/hold.rs` - the compilation capability: the per-acquisition nonce a hold hands its descendants, its published hash, and `stamp()`, applied at the spawn choke points (last, after caller env, so nothing displaces it). Replaces the guard's `/proc` ancestry inference, which was spoofable by naming any binary `brokkr` and failed wherever process shape is invisible - in the worst direction, refusing brokkr itself. Per *acquisition*, not per process: pid+starttime+boot_id names the process, so a child of a released hold would still match a later one
- `src/bin/rustc_guard.rs` + `src/guard.rs` - the rustc-wrapper fence: `brokkr-rustc-guard` is a second bin target (self-contained; the package sets `autobins = false` because both bins are explicit) that cargo invokes as `build.rustc-wrapper` once `brokkr guard --install` writes that line into `$CARGO_HOME/config.toml`. Runtime decision per rustc invocation: `BROKKR_CARGO` passes (checked before any lease); else take a *shared* lease on `~/.brokkr/compile.lock` with `FD_CLOEXEC` cleared so it rides through `exec` and is held by the compiler itself, read the published capability hash under that lease (lock first, then check - a new hold cannot republish without the exclusive lock the share excludes), and probe `brokkr.lock` **last** as the sole authority on whether a hold exists at all, since a crashed brokkr leaves a well-formed record behind and deciding on it refused every compile on the machine. Holder and guard fail in opposite directions on purpose: the holder fails acquisition without a drain barrier, the guard fails open on broken lease infrastructure. The guarantee is conditional - see `docs/commands/check.md`. `src/guard.rs` is the comment-preserving `toml_edit` config editor (install/remove/status), refusing to touch a wrapper brokkr did not write (sccache, say)
...
- `check` / `test` - validation pipeline. Its `publish_cycle` phase needs no config and ignores `-p`: it shares `deps`' cycle detection so a publication cycle fails `check` too. In a git repo where everything uncommitted is markdown it runs `gremlins`/`textlint`/`script_check` only (`scope::dirt`); a clean tree still runs everything, and `--force-rust`/`--gate`/a profile/any build-shaping flag waives the shortcut. See `docs/commands/check.md`.
...
- `run [NAME] [--debug|--release] [-- ARGS...]` / `install [--debug|--release]` - metadata-driven runnable commands (any project, no config required). `run` discovers every workspace bin + example target from `cargo metadata` and runs NAME; bare form runs `[bin] default` or the sole runnable and lists candidates otherwise (bare-is-an-index). `install` runs `cargo install --locked --path <pkg dir>` for the `[bin] install` packages, or the sole bin-carrying package - the session-workflow closer. `--locked` is the default because `cargo install --path` otherwise ignores the workspace lockfile and re-resolves, which is how a green `check` and a broken install coexist; `--unlocked` opts out, and a tree with no lockfile re-resolves with a printed warning. Profile: `--debug`/`--release` > `[bin] debug` > release (both). Same lock + `disable_toolchain` window as the build paths. See `docs/commands/run.md`.
...
- `strays [--kill]` - list cargo-family processes (cargo/rustc/rustdoc/clippy-driver/build scripts) with no brokkr ancestor, attributed to their starter; `--kill` SIGKILLs them leaves-first plus rust-analyzer when it is the starter (never a shell). Every locked command runs the same reap once it holds the lock (`src/stray.rs`, hooked in `context::acquire_cmd_lock_opt`) - no cargo runs outside brokkr on a brokkr host. See `docs/commands/check.md` (Strays).
- `guard [--install|--remove]` - manage the rustc-wrapper fence: bare shows status, `--install` enrolls `brokkr-rustc-guard` as `build.rustc-wrapper` in `$CARGO_HOME/config.toml` (the prevention side of the stray reap - a cargo carrying no capability for the current hold fails at its `rustc -vV` probe), `--remove` unsets it. See `docs/commands/check.md` (The rustc guard).
- `kill [--hard]` - cooperatively terminate the brokkr process holding the lock. Default sends SIGTERM (graceful: SIGKILLs child, flushes partial sidecar data under `dirty` alias, releases lock, runs `brokkr clean`). `--hard` sends SIGKILL to brokkr + child + mocks. Every target is identity-verified (starttime token + boot id) and signalled via pidfd (PG leaders re-verified before `kill(-pid)`); verification is all-or-nothing before the first signal, and an unverifiable target (e.g. a holder in a PID-namespace sandbox) refuses the whole kill with no signals sent. Exits 130 on graceful path.
