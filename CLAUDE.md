...
- `docs/brokkr.toml.md` - read if the user asks about config fields, host sections, the `[gremlins]` exclude list, `[[check]]`, `[test]` profiles, `[header]`, `[[textlint]]`, `[[script_check]]`, `[manifest]`, `[bin]`, `[clippy]`, or `[[quarantine]]`. Schema-universal sections only; project-specific blocks live in their own docs (below).
...
- `docs/commands/check.md` - read if working on `brokkr check` or `brokkr test`, the gremlins/clippy/test pipeline, sweep selection, profile resolution, libtest filters, or the `BROKKR_TEST_BIN_DIR` contract.
...
- `docs/projects/mogwai.md` - read if the project is mogwai and the user asks about `brokkr mogwai` - the `[mogwai]` config block, the frozen-invocation workload contract, why rows keep `brokkr_args` empty, the per-workload `timing` declaration (`external` default vs `self_reported` + its required `timing_reason`), `successor` retirement lineage, `identity_counters` and the `WORK CHANGED` / `counters:` / `TIMING CHANGED` annotations, or corpus workloads (`[<host>.corpus.*]`, the `{corpus}` token, xxh128-not-sha256).
...
- `src/mogwai/` - `brokkr mogwai`, the layer-1 regression bench. `workload.rs` (the `[mogwai.workloads.*]` registry: a name -> a frozen argv, retired entries refusing to run and naming their successor, plus `{corpus}` substitution against the per-host hash-pinned `[<host>.corpus.*]` registry), `cmd.rs` (build the `[mogwai] package` bin, then dispatch on the entry's `timing`: `run_external_with_counters` for the `external` default - brokkr's wall plus stderr counters, so a baseline can be recorded retroactively at any commit whose CLI still parses the argv - or `run_external_with_kv` for `self_reported`, which narrows the window at the cost of that back-fill and therefore requires a `timing_reason`). Rows file under the workload name with `brokkr_args` empty because it is a `pair_key` component; `input_file` carries the corpus key or nothing. `meta.identity_counters` rides on every row (written even when empty - its presence is what opts the row into the counter contract) and `meta.timing` records the clock; together they drive `--compare`'s `WORK CHANGED`, non-fatal `counters:`, and `TIMING CHANGED` annotations (`src/db/format/compare.rs`). See `docs/projects/mogwai.md`.
...
- `env` - hostname, kernel, governor, memory, drives, tool versions, dataset status, and this host's `[<host>.corpus.*]` archives. An entry with no `xxh128` prints the digest computed from the path, which is how a corpus registration gets its digest (brokkr hashes xxh128; delivery manifests carry sha256, so there is nothing to transcribe). A digest that cannot be computed reports why, not a bare `error`. A corpus `path` may be a DIRECTORY - it digests as the sorted fold of `<relpath>\0<file digest>` over the tree (`preflight::compute_xxh128_tree`), since a delivery is often a directory the consuming CLI takes whole.
...
- `results` - query the results database (`.brokkr/results.db`): bare table, `--commit`/`--command`/`--mode`/`--dataset`/`--meta`/`--env` filters, `--grep`/`--grep-v` over the whole invocation (argv plus captured env - the only way to select an env-gated arm, or an arm defined by an *absent* flag), `<uuid>` for one run, `--compare A B` for a paired delta. Uniform everywhere except litehtml. See `docs/commands/results.md`.
  For rows that opted into the counter contract (those carrying `meta.identity_counters`, today only `brokkr mogwai`), `--compare` adds three annotations: `WORK CHANGED` when a counter the workload declared identity-bearing moved - the seeded-work check that catches doing less work and reading the shorter wall as a speedup; a non-fatal `counters:` line for the undeclared ones that moved, which is the context that makes the delta readable; and `TIMING CHANGED` when the two rows recorded different clocks (`meta.timing`). All three read declarations off the row, so re-declaring cannot rewrite what an old comparison asserted.
...
- `mogwai [WORKLOAD] [MODE]` - **[mogwai]** the layer-1 regression bench over frozen invocations. Bare lists the registry; a name runs it. A workload is its argv stated in full, and its name is a promise that its rows are comparable across months - changing an invocation means a new name plus a `successor` pointer, never a quiet edit. See `docs/projects/mogwai.md`.
