...
- `docs/projects/elivagar.md` - **read when** working on elivagar-specific commands, incl. `regress`/`bless` (output-regression diff vs a blessed archive) and the durable tilegen output store.
...
- `src/elivagar/` - benchmarks, verify, compare-tiles, download-ocean, hotpath, plus `provenance.rs` (reads an archive's `elivagar` metadata block; backs `regress`'s comparability gate, which refuses an incomparable diff with exit 2). See `docs/projects/elivagar.md`.
...
- `pmtiles-stats` - **[elivagar, nidhogg]** PMTiles v3 file statistics (zoom distribution, tile sizes, compression). `src/pmtiles.rs` also exposes `read_metadata()` for the archive's metadata JSON.
