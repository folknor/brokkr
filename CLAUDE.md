...
- `docs/commands/sync.md` - **read when** the project is ratatoskr and the user asks about `sync` or `mock-serve`. `brokkr sync` is one command with three shapes (bare lists, `<SCRIPT>` runs, `<SCRIPT> --bench [N]` measures), replacing the former `sync-list`/`sync-smoke`/`sync-bench` triple. Covers sæhrimnir orchestration, readiness sentinel parsing, endpoint env-var export, marker FIFO usage, and the bench path's `--commit` split-tree rule (harness from the worktree; script, fixture and sæhrimnir from the current tree - dellingr's rule, not sluggrs').
