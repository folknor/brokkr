...
- `docs/commands/sync.md` - **read when** the project is ratatoskr and the user asks about `mock-serve`, `sync-list`, `sync-smoke`, or `sync-bench`. Covers sæhrimnir orchestration, readiness sentinel parsing, endpoint env-var export, marker FIFO usage, and sync-bench's `--commit` split-tree rule (harness from the worktree; script, fixture and sæhrimnir from the current tree - dellingr's rule, not sluggrs').
- `docs/commands/ratatoskr-gate.md` - **read when** the project is ratatoskr and the user asks about `--gate`, `--as-baseline`, the `[ratatoskr.gate.*]` config block, baseline pinning per hostname, gate.db (and its exemption from `brokkr clean`), bisecting a breach with `--commit`, or sync-bench regression thresholds (max/max_relative/max_delta/min/min_relative/equal/equal_to_baseline).
