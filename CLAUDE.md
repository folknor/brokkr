...
- `docs/projects/mogwai.md` - read if the project is mogwai and the user asks about `brokkr mogwai` - the `[mogwai]` config block, the frozen-invocation workload contract, why rows keep `input_file`/`brokkr_args` empty, external vs self-reported timing, `successor` retirement lineage, or `identity_counters`.
...
- `src/mogwai/` - `brokkr mogwai`, the layer-1 regression bench. `workload.rs` (the `[mogwai.workloads.*]` registry: a name -> a frozen argv, with retired entries refusing to run and naming their successor), `cmd.rs` (build the `[mogwai] package` bin, then `run_external` - external wall-clock, so a baseline can be recorded retroactively at any commit whose CLI still parses the argv). Rows are filed under the workload name with `input_file` and `brokkr_args` both empty, the latter because it is a `pair_key` component. See `docs/projects/mogwai.md`.
