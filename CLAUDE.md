...
- `env` - hostname, kernel, governor, memory, drives, tool versions, dataset status, and this host's `[<host>.corpus.*]` archives. An entry with no `xxh128` prints the digest computed from the file, which is how a corpus registration gets its digest (brokkr hashes xxh128; delivery manifests carry sha256, so there is nothing to transcribe).
