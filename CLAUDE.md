...
- `docs/commands/check.md` - read if working on `brokkr check` or `brokkr test`, the gremlins/clippy/test pipeline, sweep selection, profile resolution, libtest filters, the `parallel = { budget = N }` concurrent-test-binaries lane (`src/check_cmd/parallel.rs`: the in-flight budget, why it counts tests rather than binaries, why the serial side is a second `[[check]]` entry rather than a key, and the whole-workspace `-Zfeature-unification` pin that keeps the `-p`-scoped fan-out on the prebuild's feature graph), the bundled-nextest groundwork (`src/check_cmd/nextest.rs`: the `(binary-id, test)` coverage pair, `MismatchReason` precedence, the universe-listing rule), the install-feature phase (`src/check_cmd/install_shape.rs`: the `[bin] install` set compiled under package-mode feature unification, the way `cargo install` resolves it), or the `BROKKR_TEST_BIN_DIR` contract.
