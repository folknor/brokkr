...
- `src/elivagar/` - benchmarks, verify, compare-tiles, download-ocean, hotpath, `regress.rs` (two-explicit-archive passthrough; no comparability gate - regress is the tier-3 attribution instrument, comparability is the caller's job), `corpus/` (the native `pmtiles-corpus` gate - brokkr owns the adjudication; see below) and `ocean_build.rs` (`ocean-build`, derives the invocation from `[<host>.tilegen.default].ocean`). See `docs/projects/elivagar.md`.
- `src/elivagar/corpus/` + `src/elivagar/eliv.rs` - the corpus gate moved out of elivagar per the `brokkr.md`/`elivagar.md` redesign: brokkr owns the digest fold, gating policy, verdicts, SVG render core, mutate and calibration, and reads archives in-process through the linked elivagar crate. `eliv.rs` is the seam - the linked-crate surface (reader/decoder/writer/typed-provenance) the gate consumes. **Scaffold state:** until elivagar exposes that public API, `eliv.rs`'s archive/decoder/writer bodies are `unimplemented!()` stubs and the render core (`render.rs`) is pending, so the gate compiles and its pure logic (fold, `digest`/`leaves` formats, baseline self-integrity, contract gating, the reordered `check` pipeline, mutate's protobuf surgery) is built and tested, but end-to-end runs wait on the crate. `check` exits 0/1/2/3 (pass / content mismatch / archive refusal / baseline trouble). Modules: `digest` (fold + formats + self-integrity), `diff` (localized deltas), `contract` (comparability policy), `manifest`, `render` (canonical SVG core, scaffold), `mutate` (calibration instrument).
