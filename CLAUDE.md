...
- `docs/brokkr.toml.md` - read if the user asks about config fields, host sections, the `[gremlins]` exclude list, `[[check]]`, `[test]` profiles, `[header]`, `[[textlint]]`, `[[script_check]]`, `[manifest]`, `[bin]`, `[lints]` (formerly `[clippy]`), `worktree_keep`, or `[[quarantine]]`. Schema-universal sections only; project-specific blocks live in their own docs (below).
