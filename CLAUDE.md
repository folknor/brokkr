...
- `docs/commands/check.md` - read if working on `brokkr check` or `brokkr test`, the gremlins/clippy/test pipeline, sweep selection, profile resolution, libtest filters, the `parallel = { budget = N }` concurrent-test-binaries lane (`src/check_cmd/parallel.rs`: the in-flight budget, why it counts tests rather than binaries, and why the serial side is a second `[[check]]` entry rather than a key), the bundled-nextest groundwork (`src/check_cmd/nextest.rs`: the `(binary-id, test)` coverage pair, `MismatchReason` precedence, the universe-listing rule), or the `BROKKR_TEST_BIN_DIR` contract.
