...
- `docs/projects/mogwai.md` - read if the project is mogwai and the user asks about `brokkr mogwai` - the `[mogwai]` config block, the frozen-invocation workload contract, why rows keep `brokkr_args` empty, external-wall-plus-counters timing, `successor` retirement lineage, `identity_counters` and the `WORK CHANGED` annotation, or corpus workloads (`[<host>.corpus.*]`, the `{corpus}` token, xxh128-not-sha256).
...
- `src/mogwai/` - `brokkr mogwai`, the layer-1 regression bench. `workload.rs` (the `[mogwai.workloads.*]` registry: a name -> a frozen argv, retired entries refusing to run and naming their successor, plus `{corpus}` substitution against the per-host hash-pinned `[<host>.corpus.*]` registry), `cmd.rs` (build the `[mogwai] package` bin, then `run_external_with_counters` - brokkr's wall plus stderr counters, so a baseline can be recorded retroactively at any commit whose CLI still parses the argv). Rows file under the workload name with `brokkr_args` empty because it is a `pair_key` component; `input_file` carries the corpus key or nothing. The declared `identity_counters` ride on the row as `meta.identity_counters` and drive `--compare`'s `WORK CHANGED` annotation (`src/db/format/compare.rs::format_counter_diff`). See `docs/projects/mogwai.md`.
...
- `wc [threshold]` - list tracked `.rs` files with more than `threshold` lines (default 800), largest first. Works in any project.
  `--compare` also annotates a pair with `WORK CHANGED` when a counter the workload declared identity-bearing moved between the two sides - the seeded-work check that catches doing less work and reading the shorter wall as a speedup. Declaration rides on the row (`meta.identity_counters`), so re-declaring cannot rewrite what an old comparison asserted.
