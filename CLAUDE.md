...
- `clean [--worktrees]` - remove scratch/temp files. On elivagar projects wipes `tilegen_tmp` (scratch) and removes the durable tilegen output archives (`<output>/<dataset>-<commit>.pmtiles`, reproducible; retention bounds normal growth). On ratatoskr projects also wipes `.brokkr/ratatoskr/` (run-N artefact dirs left by failed runs, plus `mock/` dirs from `mock-serve`). On piners projects removes the `.brokkr/piners/corpus/run-N/` dirs but **spares `runs.db`** (the corpus run store is the source of truth). `--worktrees` also purges all persistent benchmark worktrees.
