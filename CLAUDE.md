...
- `src/bin/rustc_guard.rs` + `src/guard.rs` - the rustc-wrapper fence: `brokkr-rustc-guard` is a second bin target (self-contained; the package sets `autobins = false` because both bins are explicit) that cargo invokes as `build.rustc-wrapper` once `brokkr guard --install` writes that line into `$CARGO_HOME/config.toml`. Runtime decision per rustc invocation: `BROKKR_CARGO` set passes, a `brokkr` ancestor in `/proc` passes (the stray definition inverted - no env threading through brokkr's spawn sites), else a held brokkr flock refuses with exit 1 before anything compiles; fail-open otherwise. Never bypassed by unsetting the wrapper - wrapper identity is in cargo's compile fingerprint, so enrollment costs one full rebuild per target dir, once. `src/guard.rs` is the comment-preserving `toml_edit` config editor (install/remove/status), refusing to touch a wrapper brokkr did not write (sccache, say)
...
- `guard [--install|--remove]` - manage the rustc-wrapper fence: bare shows status, `--install` enrolls `brokkr-rustc-guard` as `build.rustc-wrapper` in `$CARGO_HOME/config.toml` (the prevention side of the stray reap - a cargo outside brokkr fails at its `rustc -vV` probe while the lock is held), `--remove` unsets it. See `docs/commands/check.md` (The rustc guard).
