...
- `src/check_cmd/binary_timings.rs` - `.brokkr/parallel-timings.toml`: per-binary wall times carried between runs so a `parallel` sweep allocates threads by measured cost rather than test count. Count-as-cost assumes every test costs the same and starves the slow-but-small binary that is the critical path. Advisory in every direction - a missing or stale file costs a worse allocation, never a wrong result
