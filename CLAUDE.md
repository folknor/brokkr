...
- `pmtiles-stats` - PMTiles v3 file statistics (zoom distribution, tile sizes, compression). `src/pmtiles.rs` also exposes `read_metadata()` for the archive's metadata JSON.
