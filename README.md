...
- `[test]` - top-level (not host-scoped). `default_package = "..."` is the cargo package `brokkr test` uses when no `-p/--package` is given; required for multi-crate workspaces that lack a built-in default. Explicit CLI `-p` always wins over this; this wins over the project's built-in default
